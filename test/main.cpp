...
int main() {
    ...
    /*example of pattern scanning for LocalPlayer and EntityList*/
    uintptr_t LocalPlayer = find_pattern(client, bytes, "8D 34 85 ? ? ? ? 89 15 ? ? ? ? 8B 41 08 8B 48 04 83 F9 FF", 0x3, 0x4);
    uintptr_t EntityList = find_pattern(client, bytes, "BB ? ? ? ? 83 FF 01 0F 8C ? ? ? ? 3B F8", 0x1, 0x0);
    delete[] bytes;
    system("pause");
}